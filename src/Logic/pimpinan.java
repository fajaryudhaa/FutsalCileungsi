...
public class pimpinan {
    ...
    public static boolean caripetugas(String nama) {
        setNama(nama);
        boolean b = false;
        String strSQL = "select * from pimpinan where nama='" + getNama()+"'";
        ...
        try {
            ...
            Logger.getLogger(pimpinan.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                getRsLpg().close();
            } catch (SQLException ex) {
    ...
    public static int updatepetugas(String trans, String[] plg) {
        ...
        if(trans.equals("tambah")) {
            strSQL = "insert into pimpinan (nama,alamat,notlp)"+
                     " values('"+plg[0]+"','"+plg[1]+"','"+plg[2]+"')";                    
        }else if(trans.equals("ubah")) {
            strSQL = "update pimpinan set alamat='"+plg[1]+"', notlp='"+plg[2]+ "'"
                    + "where nama = '"+plg [0]+"'";
        } else {
            strSQL = "delete from pimpinan where nama='"+plg[0]+"'";
        }
